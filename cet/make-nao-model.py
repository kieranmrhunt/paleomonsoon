...
cet_df = pd.read_csv("nao_station_djfm.txt", delim_whitespace=True)
...
train_period = (1865, 1930)
test_period = (1930, 1959)


y_train = cet_df[(cet_df['year'] >= train_period[0]) & (cet_df['year'] <= train_period[1])]['index'].values #* 3600 * 24
y_test = cet_df[(cet_df['year'] >= test_period[0]) & (cet_df['year'] <= test_period[1])]['index'].values #* 3600 * 24
...
early_stop = EarlyStopping(monitor='val_loss', patience=1000, verbose=1, restore_best_weights=True)
