...
data_pages2k   = lipd.readLipd('/home/users/rz908899/mitre/paleo/pages2k/')
data_pages2k_ts = lipd.extractTs(data_pages2k)

data_pages2k_ts_filtered = lipd.filterTs(data_pages2k_ts,'maxYear > 1989')
data_pages2k_ts_filtered = lipd.filterTs(data_pages2k_ts_filtered,'geo_meanLat > 0')
data_pages2k_ts_filtered = lipd.filterTs(data_pages2k_ts_filtered,'geo_meanLat < 45')
data_pages2k_ts_filtered = lipd.filterTs(data_pages2k_ts_filtered,'geo_meanLon > 50')
data_pages2k_ts_filtered = lipd.filterTs(data_pages2k_ts_filtered,'geo_meanLon < 120')
...
series_list = [create_series_from_dict(d) for d in data_pages2k_ts_filtered if 'year' in d.keys()]
df_data = pd.concat(series_list, axis=1)
df_data = df_data.reindex(index=range(1000, 2021)).sort_index()


# Extracting metadata
metadata_keys = ['mode', 'time_id', 'archiveType', 'geo_meanLon', 'geo_meanLat', 'geo_meanElev', 'geo_type', 'collectionName', 'createdBy', 'dataSetName', 'hasPaleoDepth', 'investigators', 'tagMD5', 'lipdverseLink', 'maxYear', 'minYear', 'nUniqueAges', 'archiveTypeOriginal', 'hasDepth', 'hasChron', 'originalDataUrl', 'datasetId', 'changelog', 'pub1_author', 'pub1_abstract', 'pub1_journal', 'pub1_pages', 'pub1_publisher', 'pub1_title', 'pub1_type', 'pub1_doi', 'pub1_year', 'pub1_citation', 
...
df_metadata = pd.DataFrame([{k: d[k] for k in metadata_keys if k in d} for d in data_pages2k_ts_filtered])
...
df_data.to_csv("pages2k_processed.csv")
df_metadata.to_csv("pages2k_processed_metadata.csv")
