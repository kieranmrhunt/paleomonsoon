...
fig, axes = plt.subplots(len(fnames), 1, figsize=(10, 10), sharey=True, gridspec_kw={'hspace': 0})  # 7 rows, 1 column of subplots

rescale = lambda y: (y + 2) / 4

# Create a dataframe to store the regional precip anomalies
years = np.arange(1500, 1996)
anomalies_df = pd.DataFrame(index=years, columns=[fname.split('-')[1].upper() if fname != 'aismr' else 'AI' for fname in fnames])
...
for idx, fname in enumerate(fnames):
	...
	# Create the combined anomalies using prcp_values where available and test_median otherwise
	combined_anomalies = np.full(years.shape, np.nan)
	combined_anomalies[np.isin(years, prcp_years)] = prcp_values[np.isin(prcp_years, years)]
	combined_anomalies[~np.isin(years, prcp_years)] = test_median[~np.isin(results_df['year'], prcp_years)]

	# Store the anomalies in the dataframe
	anomalies_df[fname.split('-')[1].upper() if fname != 'aismr' else 'AI'] = combined_anomalies
	...
	model_years = results_df['year']
	ix = model_years<=prcp_years[0]
	
	ax.bar(results_df['year'][ix], test_median[ix], color=cmaps.BlueYellowRed_r(rescale(test_median)),)

	ax.axvline(prcp_years[0], color='g', label = "start of instrumental record")
	
	ax.bar(prcp_years, prcp_values, color=cmaps.BlueYellowRed_r(rescale(prcp_values)),)
	...
	# Capitalize and format the subplot titles
	label = fname.upper().split('-')[1] if fname != 'aismr' else 'AI'
	ax.text(0.99, 0.025, f"{label} (r = {r_value:.3f})", transform=ax.transAxes, ha="right", va="bottom", fontsize=10, fontweight="bold")
...
ax.set_xlabel('Year')
fig.text(0.075, 0.5, 'Standardised summer precipitation', ha='center', va='center', rotation='vertical')
# Set legend
import matplotlib.patches as mpatches
handles, labels = ax.get_legend_handles_labels()
famine_patch = mpatches.Patch(color='grey', alpha=0.33, label='Famine', zorder=0)
...
anomalies_df.to_csv("../data-for-zenodo/timeline_model_regional_precip_anomalies.csv")
